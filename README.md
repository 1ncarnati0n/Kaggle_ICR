...
- 3가지 질병 중 하나 이상을 가지고 있는지(클래스 1) 또는 아무 질병도 가지고 있지 않는지(클래스 0)로 구분 (이진 분류 문제)
...
## 평가지표
$$\displaystyle\text{Log Loss}=\cfrac{-\frac{1}{N_0}\sum_{i=1}^{N_0} y_{0i}\log{p_{0i}} - \frac 1{N_1}\sum_{i=1}^{N_1} y_{1i}\log{p_{1i}}}{2} $$  
$$ p =\max(\min(p,1-10^{-15}), 10^{-15}) $$
- 환자와 비 환자의 Logloss을 각각 계산하여 그 값의 평균을 반환
- Logloss는 다른 평가지표와 비교했을 때, 우연히 정답을 맞추는 것을 방지하기 위하여 틀린 값에 대해 페널티를 주.
